	...
	public PikachuLine checkTwoPoint(Point p1, Point p2) {
		if (!p1.equals(p2) && matrix[p1.x][p1.y] == matrix[p2.x][p2.y]) {
			...
			if ((t = checkRectX(p1, p2)) != -1) {
				System.out.println("rect x");
				return new PikachuLine(new Point(p1.x, t), new Point(p2.x, t));
			}

			// check in rectangle with y
			if ((t = checkRectY(p1, p2)) != -1) {
				System.out.println("rect y");
				return new PikachuLine(new Point(t, p1.y), new Point(t, p2.y));
			}
			// check more right
